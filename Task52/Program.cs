...
double[] AverageValueColumns(int[,] matrix)
{
    double[] average = new double[matrix.GetLength(1)];
    for (int i = 0; i < matrix.GetLength(1); i++)
    {
        for (int j = 0; j < matrix.GetLength(0); j++) average[i] += matrix[j, i];
...
double[] averageArrey = AverageValueColumns(array2D);
PrintArray(averageArrey);
